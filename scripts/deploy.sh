...
# ---------------------------------------------------------------- standing probes
#
# Extracted so BOTH paths can run them, and that is the whole point of the change.
#
# These ask about the BOX, not about the release: is /metrics public, does the CSP permit
# the Sentry DSN, is there disk left, are the ops credentials set, is the reviewer account
# the demo one. None of that is decided by which commit is running — it is decided by `.env`,
# by the container that was last recreated, and by whoever touched the machine.
#
# They used to live inside `if health_ok`, reachable only after a deploy that actually
# shipped something. A deploy whose commit is already contained in the running one exits at
# line ~91 with "nothing to deploy" — green, and zero probes asked.
#
# That is not hypothetical: a CSP fix applied by hand to the box was reverted by the next
# `git reset --hard`, and the deploy after it was green while the browser silently dropped
# every crash report. Nothing asked, so nothing said.
standing_probes() {
...
if [ -n "${DEPLOY_SHA:-}" ]; then
  ...
  if git merge-base --is-ancestor "$NEW_SHA" "$PREV_SHA" 2>/dev/null; then
    log "$NEW_SHA is already contained in the running $PREV_SHA — nothing new to ship"
    # But the BOX is still worth asking about. The probes below examine `.env`, the running
    # containers and the public surface — none of which is decided by which commit is
    # running. Skipping them here made "nothing to deploy" mean "nothing checked", and a
    # green run that asked no questions is indistinguishable from one that asked and liked
    # the answers.
    #
    # Measured the hard way: a CSP fix applied by hand to the box was undone by the next
    # `git reset --hard`, and every deploy after it was green while the browser dropped every
    # crash report. Nothing asked, so nothing said.
    standing_probes
    log "no code changed; the checks above describe the box as it stands"
    exit 0
  fi
