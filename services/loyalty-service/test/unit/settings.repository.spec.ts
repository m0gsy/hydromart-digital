...
describe('SettingsPrismaRepository', () => {
  it('loadAll maps rows to SettingRow shape', async () => {
    const prisma = {
      serviceSetting: {
        findMany: async () => [
          { scope: 'GLOBAL', depotId: null, key: 'earnRateRupiah', value: '1000' },
        ],
      },
    } as never;
  ...
  it('upsert creates a new row when none exists yet (global)', async () => {
    ...
    expect(calls[0].arg).toEqual({
      where: { scope: 'GLOBAL', depotId: null, key: 'earnRateRupiah' },
      select: { id: true },
    });
  ...
  it('remove deletes matching rows', async () => {
    ...
    const repo = new SettingsPrismaRepository(prisma);
    await repo.remove('DEPOT', 'd1', 'pointExpiryMonths');

    expect(calls).toEqual([{ where: { scope: 'DEPOT', depotId: 'd1', key: 'pointExpiryMonths' } }]);
  });
});
