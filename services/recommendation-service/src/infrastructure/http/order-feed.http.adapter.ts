...
import { IngestCommand, IngestItem } from '../../application/ports/recommendation.repository';
...
/**
 * Consumes order-service's `GET /api/v1/orders/internal/completed` feed (built in a later
 * task; this adapter only relies on its documented shape). Fails open to an empty page on
 * missing config, non-2xx, timeout, or network error — the rebuild loop just stops early
 * rather than crashing; a subsequent rebuild run picks up where it left off via the cursor.
 */
@Injectable()
export class OrderFeedHttpAdapter implements OrderFeedPort {
  constructor(private readonly config: RecommendationConfigService) {}
