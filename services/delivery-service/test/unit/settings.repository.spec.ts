...
describe('SettingsPrismaRepository', () => {
  ...
  it('upsert creates a new row when none exists yet (global)', async () => {
    ...
    expect(calls[0].arg).toEqual({
      where: { scope: 'GLOBAL', depotId: null, key: 'fee' },
      select: { id: true },
    });
    expect(calls[1].arg).toEqual({
      data: { scope: 'GLOBAL', depotId: null, key: 'fee', value: '2000', updatedBy: 'u1' },
    });
  });
  ...
  it('upsert updates the existing row when one already exists (depot)', async () => {
    ...
    const repo = new SettingsPrismaRepository(prisma);
    await repo.upsert({ scope: 'DEPOT', depotId: 'd1', key: 'fee', value: '3000', updatedBy: 'u2' });
  ...
  it('remove deletes matching rows', async () => {
    ...
    const repo = new SettingsPrismaRepository(prisma);
    await repo.remove('DEPOT', 'd1', 'fee');

    expect(calls).toEqual([{ where: { scope: 'DEPOT', depotId: 'd1', key: 'fee' } }]);
  });
});
