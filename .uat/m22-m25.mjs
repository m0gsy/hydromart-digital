...
export async function run(ctx) {
  ...
  await check('UAT-M23-08', async () => {
    ...
    const u = await api('PUT', `${ADM}/retention/${rows[0].id}`, { token: ctx.admin, body: { windowLabel: rows[0].windowLabel ?? 'UAT', windowDays: (rows[0].windowDays ?? 30) + 1, seenUpdatedAt: rows[0].updatedAt } });
