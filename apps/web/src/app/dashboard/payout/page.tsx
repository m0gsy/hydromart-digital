...
function BalanceCard({ summary, onWithdrawn }: { summary: PayoutSummary; onWithdrawn: () => void }) {
  ...
  async function withdraw() {
    ...
    if (!Number.isFinite(value) || value <= 0) {
      setError('Masukkan jumlah pencairan yang valid.');
      return;
    }
    setBusy(true);
    setError(null);
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Pencairan gagal diproses.');
    } finally {
      setBusy(false);
    }
  }
  ...
  if (done) {
    return (
      <Card className="flex flex-col items-center gap-3 p-7 text-center">
        ...
        <h2 className="text-xl font-extrabold tracking-tight">Pencairan diproses</h2>
        <p className="text-sm text-muted">
          <Money amount={done.amount} className="font-bold text-[color:var(--text)]" /> dikirim ke{' '}
          {done.bankAccountRef}.
        </p>
        <div className="mt-1 w-full rounded-xl border border-app p-3 text-left text-xs">
          ...
          <div className="flex items-center justify-between py-0.5">
            ...
            <span className="font-semibold text-green-700">Diproses</span>
          </div>
        </div>
      </Card>
    );
      ...
      <div className="rounded-2xl bg-gradient-to-br from-[#0b4d57] to-[#0c97ac] p-4 text-white">
        ...
        <div className="mt-3 flex items-center gap-2 rounded-xl bg-white/15 px-3 py-2.5">
          ...
          <div className="leading-tight">
            ...
            <p className="text-[10.5px] text-white/70">a.n. akun terdaftar</p>
          </div>
        </div>
      </div>

      <Field label="Jumlah pencairan (IDR)" htmlFor="wd-amount">
        <Input
          id="wd-amount"
          inputMode="numeric"
          value={amount}
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center gap-2.5">
        ...
        <h1 className="text-2xl font-bold">Payout &amp; komisi</h1>
        <Badge tone="warning">Usulan</Badge>
      </div>
      <p className="text-sm text-muted">
        Buku kas depot, komisi HQ, dan pencairan untuk pemilik waralaba.
      </p>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Stat label="Saldo tersedia" value={formatIDR(data.availableBalance)} />
        <Stat label="Pendapatan bulan ini" value={formatIDR(data.monthRevenue)} />
        <Stat label="Komisi HQ" value={formatIDR(data.monthCommission)} />
        <Stat label="Payout berikutnya" value={nextPayout} />
      </div>

      <div className="grid items-start gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
        <Card className="flex flex-col p-5">
          <h2 className="mb-1 font-semibold">Buku kas</h2>
          {data.recentEntries.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted">Belum ada transaksi.</p>
          ) : (
  ...
  if (!canViewPayout(customer?.role)) {
    return (
      <CenterState title="Franchise owners only" icon={<Lock size={40} weight="fill" />}>
        Payout &amp; komisi tersedia untuk pemilik waralaba.
      </CenterState>
    );
  }
  return <PayoutBody />;
