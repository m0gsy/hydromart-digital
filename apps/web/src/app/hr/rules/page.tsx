...
function RulesBody() {
  ...
  const depotName = (id: string | null) => (id ? depots.data?.items.find((d) => d.id === id)?.name ?? t('hrFix.rules.depot') : t('hrFix.rules.allDepots'));
    ...
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      {rules.loading ? <Skeleton className="h-40" /> : rules.error ? <ErrorState message="Gagal memuat rule" onRetry={rules.reload} /> : (
        <Card className="divide-y divide-[color:var(--border)]">
          {(rules.data ?? []).length === 0 && <p className="p-5 text-sm text-muted">{t('hrFix.rules.empty')}</p>}
          {(rules.data ?? []).map((r) => (
            <div key={r.id} className="flex items-center justify-between gap-3 p-4">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-semibold">{r.name}</span>
                  <Badge tone={r.active ? 'success' : 'neutral'}>{r.active ? t('hrFix.rules.active') : t('hrFix.rules.inactive')}</Badge>
      ...
      {admin && (
        <form onSubmit={submit}>
          <Card className="grid gap-4 p-5 sm:grid-cols-2">
            ...
            <Field label={t('hrFix.rules.ruleName')}><Input value={form.name} onChange={(e) => set('name', e.target.value)} placeholder={t('hrFix.rules.ruleNameHint')} /></Field>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Field label={t('hrFix.rules.threshold')}><Input type="number" value={form.threshold} onChange={(e) => set('threshold', e.target.value)} /></Field>
            </div>
            ...
            <Field label={form.rewardKind === 'FIXED' ? t('hrFix.rules.valueRp') : t('hrFix.rules.percent')}><Input type="number" value={form.rewardValue} onChange={(e) => set('rewardValue', e.target.value)} /></Field>
            ...
            <div className="col-span-full"><Button type="submit" loading={saving}>{t('hrFix.rules.addRule')}</Button></div>
          </Card>
        </form>
      )}
    </div>
