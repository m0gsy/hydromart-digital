...
// Only these roles resolve depots through the chain — a STAFF_DEPOT or KEPALA_DEPOT is
// locked to the depot on their token and has nothing to record here.
const SUPERVISING_ROLES = ['ASSISTANT_SUPERVISOR', 'SUPERVISOR', 'MANAGER'] as const;
...
export default function HqHierarchyPage() {
  ...
  const staff = useAsync<Page<Customer>>(
    () => api.get(endpoints.auth.staff({ limit: 200 }), true),
    [],
  );
  ...
  // Candidate superiors: anyone in the chain except the person being edited. The server
  // rejects a cycle anyway; this only keeps the obvious mistake off the list.
  const superiors = people.filter(
    (p) => p.id !== staffId && (SUPERVISING_ROLES as readonly string[]).includes(p.role),
  );
