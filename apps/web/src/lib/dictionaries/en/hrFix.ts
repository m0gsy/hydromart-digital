/** HR console — EN mirror of id/hrFix.ts. Same shape, key for key. */
export const hrFix = {
  notifPrefs: {
    saveFailed: 'Could not save {label}',
  },
  myHome: {
    greeting: 'Hi, {name}',
    employee: 'Employee',
  },
  map: {
    historyChange: {
      HIRED: 'Hired',
      ACCOUNT_LINKED: 'Login account linked',
      employmentStatus: 'Employment status',
      position: 'Position',
      role: 'Login role',
      status: 'Status',
      exitDate: 'Exit date',
      salaryType: 'Pay type',
      dailyRate: 'Daily rate',
      monthlyRate: 'Monthly salary',
      depotId: 'Depot',
    },
    salaryType: { DAILY: 'Daily', MONTHLY: 'Monthly' },
    bonusType: {
      ATTENDANCE: 'Attendance',
      PERFORMANCE: 'Performance',
      SALES: 'Sales',
      DEPOT: 'Depot',
      MANUAL: 'Manual',
    },
    deductionType: {
      LATE: 'Late arrival',
      ABSENCE: 'Absence',
      MANUAL: 'Manual',
      CASH_ADVANCE: 'Cash advance',
      OTHER: 'Other',
    },
    gender: { MALE: 'Male', FEMALE: 'Female' },
    ptkp: {
      TK0: 'TK/0 — single',
      TK1: 'TK/1 — single, 1 dependant',
      TK2: 'TK/2 — single, 2 dependants',
      TK3: 'TK/3 — single, 3 dependants',
      K0: 'K/0 — married',
      K1: 'K/1 — married, 1 dependant',
      K2: 'K/2 — married, 2 dependants',
      K3: 'K/3 — married, 3 dependants',
    },
    leaveType: {
      ANNUAL: 'Annual leave',
      SICK: 'Sick leave',
      PERMISSION: 'Personal leave',
      EMERGENCY: 'Emergency',
    },
    leaveStatus: {
      PENDING_MANAGER: 'Awaiting manager',
      PENDING_HR: 'Awaiting HR',
      APPROVED: 'Approved',
      REJECTED: 'Rejected',
      CANCELLED: 'Cancelled',
    },
    kasbonStatus: {
      PENDING: 'Awaiting a decision',
      APPROVED: 'Approved',
      REJECTED: 'Rejected',
      CANCELLED: 'Cancelled',
    },
    docType: {
      KTP: 'National ID (KTP)',
      KK: 'Family card (KK)',
      CONTRACT: 'Employment contract',
      NPWP: 'Tax number (NPWP)',
      CERTIFICATE: 'Certificate',
      SIM: 'Driving licence',
      OTHER: 'Other',
    },
    employmentStatus: { TRAINING: 'Training', PROBATION: 'Probation', PERMANENT: 'Permanent' },
    role: {
      STAFF_DEPOT: 'Depot staff / courier',
      KEPALA_DEPOT: 'Depot head',
      ASSISTANT_SUPERVISOR: 'Assistant supervisor',
      SUPERVISOR: 'Supervisor',
      MANAGER: 'Manager',
    },
    employeeStatus: { ACTIVE: 'Active', INACTIVE: 'Inactive', RESIGNED: 'Resigned' },
    attendance: {
      PRESENT: 'Present',
      LATE: 'Late',
      ABSENT: 'Absent',
      LEAVE: 'On leave',
      HOLIDAY: 'Holiday',
      PENDING: 'Awaiting approval',
    },
    payrollStatus: { DRAFT: 'Draft', APPROVED: 'Approved', PAID: 'Paid' },
    allowanceType: {
      TRANSPORT: 'Transport',
      MEAL: 'Meals',
      POSITION: 'Position',
      HOUSING: 'Housing',
      OTHER: 'Other',
    },
    assetType: {
      MOTORCYCLE: 'Motorcycle',
      SMARTPHONE: 'Phone',
      UNIFORM: 'Uniform',
      LAPTOP: 'Laptop',
      PRINTER: 'Printer',
      SCANNER: 'Scanner',
      OTHER: 'Other',
    },
    assetStatus: {
      AVAILABLE: 'Available',
      ASSIGNED: 'In use',
      RETURNED: 'Returned',
      MAINTENANCE: 'Under repair',
      LOST: 'Lost',
    },
    assetMove: {
      ASSIGN: 'Handover',
      TRANSFER: 'Transfer',
      RETURN: 'Return',
      MAINTENANCE: 'Sent for repair',
      LOST: 'Declared lost',
    },
    announceLevel: { INFO: 'Information', WARNING: 'Attention', URGENT: 'Urgent' },
    announceDim: {
      COMPANY: 'Whole company',
      DEPOT: 'Depot',
      DEPARTMENT: 'Department',
      POSITION: 'Position',
      EMPLOYEE: 'Specific employee',
    },
    bonusMetric: {
      ATTENDANCE_RATE: 'Attendance rate (%)',
      PRESENT_DAYS: 'Days present',
      ZERO_LATE: 'Never late (1=yes)',
      IS_DEPOT_MANAGER: 'Depot head (1=yes)',
      SALES_TOTAL: 'Depot sales total (Rp)',
    },
    rewardKind: { FIXED: 'Fixed amount (Rp)', PERCENT: '% of base salary' },
    weekday: {
      0: 'Sunday',
      1: 'Monday',
      2: 'Tuesday',
      3: 'Wednesday',
      4: 'Thursday',
      5: 'Friday',
      6: 'Saturday',
    },
  },
  common: {
    periodLabel: 'Period {period}',
    notSet: 'Not set',
    years: '{n} years',
    readRate: '{read} of {total} read ({pct}%)',
    all: 'All',
    allDepots: 'All depots',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    note: 'Note',
    name: 'Name',
    date: 'Date',
    amount: 'Amount',
    status: 'Status',
    type: 'Type',
    employee: 'Employee',
    depot: 'Depot',
    period: 'Period',
    empty: 'No data yet.',
    saveFailed: 'Could not save.',
    optional: 'Optional',
  },
  form: {
    roleRequired: 'Position (login role) is required.',
    fieldRequired: '{field} is required.',
    dailyRateRequired: 'Daily rate (dailyRate) must be greater than 0.',
    monthlyRateRequired: 'Monthly rate (monthlyRate) must be greater than 0.',
    nikDigits: 'National ID must be 16 digits.',
    contractBeforeJoin: 'Contract end cannot be before the join date.',
    exitBeforeJoin: 'Exit date cannot be before the join date.',
  },
  assets: {
    recordMovement: 'Record movement',
    valueLabel: 'Value',
    movedTo: ' · to {name}',
    movedCondition: ' · condition: {condition}',
    movedFrom: ' · from {name}',
    editDetail2: 'Edit details',
    kind2: 'Type',
    saveDetail: 'Save details',
    cancel2: 'Cancel',
    writtenOff: 'This asset is written off. If it turns up, register it as a new asset.',
    editDetail: 'Edit details',
    title: 'Assets',
    subtitle: 'Company property held by staff. The handover history is never deleted.',
    all: 'All',
    empty: 'No assets match.',
    noMoves: 'No movements since the asset was registered.',
    pickEmployee: 'Select an employee',
    holderUnknown: 'Holder not found',
    holderLeft: '{name} (has left)',
    register: 'Register asset',
    registerSubmit: 'Register',
    pickDepot: 'Select a depot',
    assetName: 'Asset name',
    brand: 'Brand',
    serial: 'Serial number',
    note: 'Note',
    movement: 'Movement',
    recipient: 'Receiving employee',
    condition: 'Condition (optional)',
    conditionHint: 'scratch on the mudguard',
    code: 'Asset code',
    name: 'Name',
    type: 'Type',
    ownerDepot: 'Owning depot',
    brandOpt: 'Brand (optional)',
    serialOpt: 'Serial number (optional)',
    valueOpt: 'Purchase value (Rp, optional)',
    close: 'Close',
    history: 'History & movements',
    moveSaved: 'Asset movement recorded',
    nameRequired: 'Asset name is required',
    detailFailed: 'Could not save the asset details',
    pickRecipient: 'Select the receiving employee',
    moveFailed: 'Could not record the movement',
    fillRequired: 'Fill in the code, name and depot',
    registered: 'Asset registered',
    registerFailed: 'Could not register the asset',
  },
  shift: {
    week: 'Week',
    assignmentHint:
      'Assignments are additive — earlier ones stay on file as history. Employees with no assignment are still judged against the depot shift, as before.',
    createRotation2: 'Create rotation',
    assign: 'Assign',
    title: 'Shifts & rotations',
    subtitle:
      "Working hours per employee. Attendance judges lateness against the employee's shift, not the depot's.",
    rotations: 'Weekly rotation patterns',
    noRotations: 'No rotation patterns yet.',
    allDepots: 'All depots',
    dayOff: 'Day off',
    assignments: 'Employee assignments',
    pickEmployee: 'Select an employee',
    neverAssigned: 'Never assigned — falls back to the depot shift.',
    fixedShift: 'Fixed shift',
    weeklyRotation: 'Weekly rotation',
    pick: 'Select…',
    rotationName: 'Rotation name',
    rotationNameHint: 'Warehouse rotation',
    depot: 'Depot',
    employee: 'Employee',
    kind: 'Type',
    effectiveFrom: 'Effective from',
    noteOpt: 'Note (optional)',
    shiftDeleted: 'shift deleted',
    nameRequired: 'Enter a rotation name',
    rotationCreated: 'Rotation created',
    createFailed: 'Could not create the rotation',
    updateFailed: 'Could not update the rotation',
    active: 'Active',
    inactive: 'Inactive',
    allDepotsLower: 'all depots',
    deactivate: 'Deactivate',
    activate: 'Activate',
    rotationDeleted: 'rotation deleted',
    noShiftSeeNote: 'no shift — see the note',
    pickBoth: 'Select an employee and a shift or rotation',
    assigned: 'Shift assignment recorded',
    assignFailed: 'Could not assign',
    shift: 'Shift',
    rotation: 'Rotation',
  },
  employeeForm: {
    phoneTakenTail: ' — not create a new account. If the number is a typo, fix it first.',
    phoneTakenSave: 'Saving will turn that account into',
    phoneTaken: 'The number {phone} already belongs to the account of',
    resignedHint:
      'A RESIGNED status does not stop pay — fill in the exit date, that is what payroll reads.',
    cancel2: 'Cancel',
    pickDepot: 'Select a depot…',
    daily: 'Daily',
    monthly: 'Monthly',
    notFilled: 'Not filled in',
    fullName: 'Full name',
    position: 'Position',
    phone: 'Phone number',
    emailOpt: 'Email (optional)',
    depot: 'Assigned depot',
    joinDate: 'Join date',
    role: 'Position (login role)',
    employmentStatus: 'Employment status',
    salaryType: 'Salary type',
    dailyRate: 'Daily rate (Rp)',
    monthlyRate: 'Monthly rate (Rp)',
    bankNameOpt: 'Bank name (optional)',
    bankAccountOpt: 'Account number (optional)',
    emergencyNameOpt: 'Emergency contact (optional)',
    emergencyPhoneOpt: 'Emergency contact number (optional)',
    departmentOpt: 'Department (optional)',
    npwpOpt: 'Tax number / NPWP (optional)',
    bpjsKesOpt: 'Health insurance / BPJS (optional)',
    bpjsTkOpt: 'Employment insurance / BPJS TK (optional)',
    nikOpt: 'National ID number (optional)',
    birthDateOpt: 'Date of birth (optional)',
    genderOpt: 'Gender (optional)',
    ptkpOpt: 'PTKP tax class (optional)',
    contractEndOpt: 'Contract end (optional)',
    exitDate: 'Exit date (leave blank if still employed)',
    status: 'Status',
    addressOpt: 'Address (optional)',
    updated: 'Employee updated',
    added: 'Employee added',
    saveFailed: 'Could not save.',
    unchanged: 'Unchanged',
    pickRole: 'Select a position…',
    notSet: 'Not set',
    pickDepotFirst: 'Select a depot first…',
    noName: '(no name)',
    chosenRole: 'the chosen position',
    useThatAccount: 'Yes, use that account',
    saveChanges: 'Save changes',
    addEmployee: 'Add employee',
  },
  calendar: {
    noShift: 'No shift yet (uses the {workStartTime} default).',
    inactive: 'inactive',
    depot: 'depot',
    title: 'Working calendar',
    subtitle: 'Holidays and shifts — used to work out lateness and absence deductions',
    holidays: 'Holidays',
    noHolidays: 'No holidays yet.',
    delete: 'Delete',
    date: 'Date',
    name: 'Name',
    add: 'Add',
    shift: 'Shift',
    start: 'Start',
    end: 'End',
    holidayHint: 'Independence Day',
    shiftHint: 'Morning',
    fillDateName: 'Enter a date and a name',
    holidayAdded: 'Holiday added',
    failed: 'Failed',
    deleted: 'Deleted',
    fillShiftName: 'Enter a shift name',
    shiftAdded: 'Shift added',
    shiftOff: 'Shift deactivated',
    shiftOn: 'Shift activated',
    depotScope: '(depot)',
    scope: 'Applies to',
    scopeAll: 'All depots',
    depotNamed: '({depot})',
    nationalScope: '(national)',
    deactivate: 'Deactivate',
    activate: 'Activate',
  },
  adjustments: {
    type: 'Type',
    kindLabel: 'Type',
    importDeductions: 'Import deductions',
    title: 'Bonuses & deductions',
    subtitle: 'Per employee, per period',
    period: 'Period',
    load: 'Load',
    bonus: 'Bonus',
    deduction: 'Deduction',
    add: 'Add',
    amount: 'Amount',
    note: 'Note',
    save: 'Save',
    remove: 'Delete',
    removeConfirm: 'Delete this adjustment? State the reason.',
    removeError: 'Could not delete the adjustment.',
    entersPayroll: 'Included in payroll when it is generated.',
    employeeIdHint: 'Employee UUID',
    fillEmployeeId: 'Enter an employeeId',
    loadFailed: 'Could not load',
    amountPositive: 'Amount must be greater than 0',
    bonusAdded: 'Bonus added',
    deductionAdded: 'Deduction added',
    saveFailed: 'Could not save',
  },
  rules: {
    edit: 'Edit',
    editRule: 'Edit rule: {name}',
    saveChanges: 'Save changes',
    cancelEdit: 'Cancel',
    updated: 'Rule updated',
    loadFailed: 'Could not load the rules',
    pctOfBase: '{pct}% of base pay',
    title: 'Automatic bonus rules',
    subtitle: 'Bonuses are worked out automatically when payroll is generated',
    empty: 'No rules yet.',
    addRule: 'Add rule',
    allDepotsGlobal: 'All depots (global)',
    ruleName: 'Rule name',
    ruleNameHint: 'e.g. Full attendance bonus',
    bonusType: 'Bonus type',
    metric: 'Metric',
    operator: 'Operator',
    threshold: 'Threshold',
    rewardKind: 'Reward type',
    appliesTo: 'Applies to',
    nameRequired: 'Rule name is required.',
    thresholdInvalid: 'The threshold is not valid.',
    rewardInvalid: 'The reward value is not valid.',
    added: 'Bonus rule added',
    saveFailed: 'Could not save.',
    toggleFailed: "Could not change the rule's status",
    depot: 'Depot',
    allDepots: 'All depots',
    active: 'Active',
    inactive: 'Inactive',
    deactivate: 'Deactivate',
    activate: 'Activate',
    valueRp: 'Amount (Rp)',
    percent: 'Percent (%)',
  },
  home: {
    payrollPeriod: 'Payroll {period}',
    attendanceToday: 'Attendance today ({date})',
    title: 'HR dashboard',
    empty: 'No data yet.',
    headcountMix: 'Active headcount mix',
    docsExpiring: 'Expired & expiring documents (30 days)',
    docsExpired: 'Past due',
    docsDaysLeft: '{days} days left',
    docsExpiringEmpty: 'Nothing needs renewing.',
    contractsEnding: 'Contracts & probations ending (30 days)',
    contractOver: 'Already over',
    gross: 'Gross',
    bonus: 'Bonus',
    deduction: 'Deductions',
    net: 'Net',
    manageEmployees: 'Manage employees →',
    runPayrollLink: 'Run payroll →',
    downloadReport: 'Download report →',
    addFirst: 'Add the first employee from the Employees menu.',
    totalEmployees: 'Total employees',
    payrollNet: 'Payroll (net)',
    runPayroll: 'Payroll runs',
    presentToday: 'Present today',
    noEmployees: 'No employees yet',
  },
  settings: {
    scopeLabel: 'Scope',
    scopeOptionGlobal: 'Network default',
    scopeOptionDepot: 'Specific depot',
    formatLabel: 'format',
    effectiveLabel: 'effective',
    pickDepot: 'Choose a depot…',
    scopeDepot: 'this depot',
    scopeGlobal: 'the whole network',
    notFilled: '(not filled in)',
    title: 'Payroll configuration',
    subtitle: 'GLOBAL defaults (SUPER_ADMIN) or a per-depot override',
    globalOnly: 'Only a SUPER_ADMIN can change the GLOBAL defaults.',
    save: 'Save',
    reset: 'Reset',
    depotIdHint: 'Depot UUID',
    noChange: 'That value has not been changed.',
    needDepotId: 'Enter a depotId for a DEPOT override',
    saved: 'Saved',
    saveFailed: 'Could not save',
  },
  departments: {
    inactive: 'inactive',
    codeLabel: 'Code',
    deleteConfirm:
      'Delete the department {name}? The server refuses while any employee still points at it.',
    emptyBody2: 'No departments yet — employees show as “Not set”.',
    delete2: 'Delete',
    name2: 'Name',
    depot2: 'Depot',
    title: 'Departments',
    subtitle: 'The unit an employee works in. No depot means it spans them all (Finance, HR).',
    allDepots: 'All depots',
    add: 'Add',
    nameHint: 'Finance',
    failed: 'Failed',
    fillCodeName: 'Enter a code and a name',
    added: 'Department added',
    deactivated: 'Deactivated',
    activated: 'Activated',
    deleted: 'Deleted',
    allDepotsLower: 'all depots',
    deactivate: 'Deactivate',
    activate: 'Activate',
  },
  reports: {
    toLabel: 'To',
    rangeHint:
      'The date range applies to attendance, lateness, leave and announcements. The period applies to payroll and performance. Long reports are truncated in the PDF, which says so on the page.',
    from2: 'From',
    period2: 'Period',
    depot: 'Depot',
    allDepots: 'All depots',
    scoreHint: 'Scores per period. A component that could not be measured shows blank, not zero.',
    title: 'Reports',
    subtitle: 'Export as CSV, Excel or PDF',
    directory: 'Employee directory',
    directoryBody: 'Every employee within your depot scope.',
    payroll: 'Payroll',
    payrollBody: 'Pay per period, with bonuses and deductions.',
    performance: 'Performance',
    assets: 'Assets',
    assetsBody: 'Every asset and who currently holds it.',
    downloadFailed: 'Download failed',
    fillRange: 'Enter a date range',
    attendance: 'Attendance',
    attendanceBody: 'Every attendance record in the date range.',
    lateness: 'Lateness',
    latenessBody: 'Only the days that were actually late, worst first.',
    leave: 'Leave',
    leaveBody: 'Includes leave that crosses the edge of the range.',
    announcements: 'Announcements',
    announcementsBody: 'Reach and read rate for each announcement.',
  },
  me: {
    backToConsole2: '← Back to the console',
    subtitle: 'Employee self-service',
    punchNow: 'Clock in now',
    punchBody: 'Check in and out with face verification',
    myAttendance: 'My attendance',
    myPayslips: 'My payslips',
    myLeave: 'My leave',
    myLeaveBody: 'Request leave and see what quota is left',
    announcements: 'Announcements',
    announcementsBody: 'News from HR for you',
    enrollFace: 'Enrol or update your face',
  },
  attendance: {
    faceScoreLabel: 'Face-match score at check-in',
    trailShow: 'Show correction history',
    trailEmpty: 'Never corrected.',
    pendingTitle: 'Attendance awaiting approval ({count})',
    pendingReason:
      'Submitted long after the clock-in time, so the hour came from the device, or captured outside every depot they are responsible for. Not counted as present yet.',
    title: 'Attendance',
    correctStatus: 'Correct the status',
    reject: 'Reject',
    approve: 'Approve',
    from: 'From',
    to: 'To',
    manualEntry: 'Manual entry or correction',
    date: 'Date',
    reason: 'Reason',
    save: 'Save',
    empty: 'No attendance records.',
    approveNote: 'Approval note (optional)',
    rejectNote: 'Reason for rejection (optional)',
    approved: 'Attendance approved',
    rejected: 'Attendance rejected',
    failed: 'Failed',
    onTime: 'on time',
    photoIn: 'Check-in photo',
    photoOut: 'Check-out photo',
    matchScore: '{score}% match',
    photoFailed: 'That attendance photo is not available.',
    unnamed: 'Anonymised employee',
    fillIdDate: 'Enter an employeeId and a date',
    manual: 'Manual entry',
    manualSaved: 'Manual attendance saved',
    adjustTitle: 'Correct this attendance record',
    adjustPrompt:
      'Change the status from {from} to {to}. The reason is filed against the attendance row and read back when payroll is questioned.',
    adjustConfirm: 'Correct it',
    corrected: 'Corrected',
  },
  employees: {
    noAccountCreate: 'No account yet · create one',
    title: 'Employees',
    searchHint: 'Search by name, code or position…',
    add: '+ Add',
    allStatuses: 'All statuses',
    allDepartments: 'All departments',
    empty: 'No employees.',
    accountFailed: 'Could not create the account.',
    accountCreated: 'Account created for {name}',
    importExcel: 'Import Excel',
    departmentUnreadable: 'Departments could not be read',
  },
  leave: {
    fileForEmployee: 'File leave for an employee',
    fileEmployee: 'Employee',
    fileType: 'Leave type',
    fileStart: 'From',
    fileEnd: 'To',
    fileReason: 'Reason',
    fileSubmit: 'File it',
    fileCancel: 'Cancel',
    fileFillAll: 'Fill in the employee, the dates and the reason.',
    filed: 'Filed — it is in the approval queue.',
    fileFailed: 'Could not file that application.',
    note: 'Note: {note}',
    workingDays: '{days} working days',
    importBalances: 'Import leave balances',
    reject2: 'Reject',
    title: 'Leave requests',
    subtitle: "Stage 1 the manager, stage 2 HR. HR's approval writes the attendance as leave.",
    noteHint: 'Note (required when rejecting)',
    noteFor: 'Note for {name}',
    allStatuses: 'All statuses',
    empty: 'No requests.',
    approve: 'Approve',
    awaitingHr: "Awaiting HR's decision.",
    approved: 'Request approved',
    rejected: 'Request rejected',
    failed: 'Could not process it',
  },
  performance: {
    attendanceScore: 'Attendance score',
    attendanceShort: 'Att',
    disciplineScore: 'Discipline score',
    disciplineShort: 'Disc',
    salesScore: 'Sales score',
    salesShort: 'Sales',
    computeScore: 'Compute score',
    saveFailed: 'Could not save',
    rowDays: '{present}/{working} days, {late}× late',
    measured:
      'Measured: attendance {attendance} · discipline {discipline} · sales {sales} · final {final}. Components that could not be measured are left out of the total.',
    dashHint:
      '“—” means the component could not be measured this period (no scheduled working days, no attendance, or no sales target set) — not a zero. Component weights are set in Payroll Configuration.',
    savedFor: 'Review for {name} saved',
    period3: 'Period',
    loadHistory: 'Load history',
    period2: 'Period',
    note2: 'Note',
    saveManual: 'Save manually',
    depot2: 'Depot',
    noActiveEmployees: 'No active employees in this scope.',
    save2: 'Save',
    title: 'Performance',
    subtitle: 'A monthly score from attendance, discipline and sales',
    empty: 'No reviews yet.',
    allDepots: 'All depots',
    employee: 'Employee',
    attendance: 'Attendance',
    discipline: 'Discipline',
    sales: 'Sales',
    score: 'Score',
    fillEmployeeId: 'Enter an employeeId',
    loadFailed: 'Could not load',
    nothingMeasurable: 'There is nothing measurable for this period yet',
    scoreFailed: 'Could not work out the score',
    scoreRange: 'Score 0–100',
    saved: 'Review saved',
  },
  employeeDetail: {
    framesReady: '{n} photos ready',
    resetFrames: 'Reset',
    role: 'Role (login)',
    depot: 'Depot',
    exitDate: 'Exit date',
    viewPayroll: 'View payroll →',
    viewAttendance: 'Attendance history →',
    enrolHint: 'Take 1–3 clear face photos for attendance verification.',
    saveEnrol: 'Save enrolment',
    enrollFace: 'Enrol face',
    history: 'Employment history',
    noHistory: 'No history yet.',
    phone: 'Phone number',
    email: 'Email',
    department: 'Department',
    joinDate: 'Join date',
    tenure: 'Length of service',
    salaryType: 'Salary type',
    salaryAmount: 'Salary amount',
    bank: 'Bank',
    emergency: 'Emergency contact',
    bpjsKes: 'Health insurance (BPJS)',
    bpjsTk: 'Employment insurance (BPJS TK)',
    nik: 'National ID number',
    birthDate: 'Date of birth',
    gender: 'Gender',
    address: 'Address',
    ptkp: 'PTKP tax class',
    contractEnd: 'Contract end',
  },
  resellers: {
    denied: 'Access denied',
    achievements: 'Achievements {month}',
    perGallon: 'Rp{amount}/gallon',
    atThisDepot: '{n} at this depot',
    addReseller2: 'Add reseller',
    save2: 'Save',
    cancel2: 'Cancel',
    edit2: 'Edit',
    depot2: 'Depot',
    title: 'Resellers (agents)',
    add: 'Add reseller',
    inactive: 'Inactive',
    dormant: 'Dormant',
    pickDepot: 'Select a depot…',
    empty: 'No resellers at this depot yet.',
    photo: 'Photo',
    phone: 'Customer phone number',
    phoneHint: 'Format 0812…, +62… or 62…',
    monthlyTarget: 'Monthly target (gallons)',
    discount: 'Reseller discount (%)',
    discountHint: '0–100, blank means 0',
    flatPrice: 'Flat price per gallon (Rp)',
    flatPriceHint: 'Fill this in to use a fixed price instead of a percentage discount',
    joinDate: 'Join date',
    discountShort: 'Discount (%)',
    flatPriceShort: 'Flat price per gallon (Rp)',
    flatPriceShortHint: '0 means use the percentage discount',
    noteOpt: 'Note (optional)',
    effectiveAt: 'Effective from (optional)',
    effectiveAtHint:
      'Blank means it applies now. A future date keeps the old price until that day.',
    scheduled: 'Change scheduled',
    nothingToSchedule: 'A date was given but no price or status change goes with it.',
    history: 'Price change history',
    historyEmpty: 'No price change has been recorded yet.',
    historyFailed: 'Could not load the change history.',
    historyPending: 'Scheduled',
    historyFieldDiscount: 'Discount (%)',
    historyFieldFlat: 'Flat price per gallon',
    historyFieldActive: 'Reseller status',
    historyActiveTrue: 'active',
    historyActiveFalse: 'inactive',
    required: 'Phone number, monthly target and join date are required.',
    targetNumber: 'The monthly target must be a number of 0 or more.',
    discountRange: 'The discount must be between 0 and 100.',
    flatNumber: 'The flat price per gallon must be a number of 0 or more.',
    added: 'Reseller added',
    phoneNotFound: 'No customer with that number was found.',
    notACustomer: 'That number does not belong to a registered customer.',
    alreadyReseller: 'This customer is already a reseller.',
    addFailed: 'Could not add the reseller.',
    updated: 'Reseller updated',
    updateFailed: 'Could not update the reseller.',
    deactivated: 'Reseller deactivated',
    reactivated: 'Reseller reactivated',
    statusFailed: "Could not change the reseller's status.",
    deactivate: 'Deactivate',
    activate: 'Activate',
    pickDepotToView: 'Select a depot to see its resellers.',
    noDepot: 'No depot has been chosen yet.',
    photoTooBig: 'The photo is larger than 5MB.',
    photoSaved: 'Agent photo saved',
    photoFailed: 'Could not upload the photo.',
    replacePhoto: 'Replace the agent photo',
    uploadPhoto: 'Upload an agent photo',
  },
  orderDetail: {
    advanceTo: 'Advance to {status}',
    notes: 'Note: {notes}',
    paymentMethod: 'Payment · {method}',
    onlineRefundHint:
      'The payment was settled when this order was cancelled: an unpaid one was voided, a paid one refunded. A high-value refund waits for HQ approval before the money moves.',
    printReceipt2: 'Print receipt',
    // B5: the delivery window the customer picked at checkout, finally on a screen.
    window: 'Delivery window',
    assignCourier: 'Assign a courier',
    loadingCouriers: 'Loading couriers…',
    noCouriers: 'No active couriers. Invite one from Staff &amp; roles.',
    pickCourier: 'Select a courier…',
    walkIn: 'Walk-in',
    items: 'Items',
    subtotal: 'Subtotal',
    deliveryFee: 'Delivery fee',
    discount: 'Discount',
    total: 'Total',
    cancelled: 'Order cancelled',
    statusHistory: 'Status history',
    proofLabel: 'Payment proof',
    proofMissing: 'Not uploaded by the customer.',
    cashReceived: 'Cash received (optional)',
    cashHint: 'Filled in means the change is recorded on the payment too.',
    courier: 'Courier',
    payFailed: 'Could not confirm the payment.',
    refundAction: 'Raise a refund',
    failAction: 'Mark as failed',
    failTitle: 'Mark this payment failed?',
    failMessage: 'The payment for {order} will be marked FAILED. The order stays, and the customer can still pay again.',
    failConfirm: 'Mark failed',
    refundTitle: 'Raise a payment refund',
    refundMessage:
      'The money for order {order} goes back to the customer. This cannot be undone; a high-value refund waits for HQ approval first.',
    refundReason: 'Refund reason',
    refundReasonHint: 'e.g. one gallon short, charged twice',
    refundConfirm: 'Raise refund',
    refundFailed: 'Could not raise the refund.',
    pickCourierFirst: 'Select a courier.',
    assignFailed: 'Could not assign the courier.',
    noShift: '— has not opened a shift',
    updateFailed: 'Could not update the order.',
  },
  vouchers: {
    giveTitle: 'Give {code} to a customer',
    cancel3: 'Cancel',
    activeVouchers: 'Active vouchers',
    cancel2: 'Cancel',
    close2: 'Close',
    search2: 'Search',
    gateBody2: 'Vouchers are for the marketing team and depot admins.',
    percentage: 'Percentage (%)',
    fixedAmount: 'Fixed amount (Rp)',
    newVoucher: 'New voucher',
    code: 'Code',
    codeHint: 'Upper-cased automatically, must be unique',
    description: 'Description',
    descriptionHint: '10% off a refill',
    discountType: 'Discount type',
    maxDiscount: 'Maximum discount (Rp)',
    maxDiscountHint: 'The ceiling for a percentage discount',
    minSpend: 'Minimum spend (Rp)',
    minSpendHint: 'Leave blank for no minimum',
    totalQuota: 'Total quota',
    totalQuotaHint: 'Leave blank for no limit',
    perCustomerQuota: 'Quota per customer',
    validFrom: 'Valid from',
    validFromHint: 'Leave blank to start immediately',
    validUntil: 'Valid until',
    validUntilHint: 'Leave blank for no end date',
    active: 'Active',
    activeHint: 'Switch off to stop the voucher',
    customerPhone: 'Customer phone number',
    phoneHint: 'Format 0812…, +62… or 62…',
    denied: 'Access denied',
    emptyTitle: 'No vouchers yet',
    codeRequired: 'The voucher code is required.',
    valuePositive: 'The discount value must be greater than 0.',
    saveFailed: 'Could not save the voucher.',
    valuePercent: 'Value (%)',
    valueRupiah: 'Value (Rp)',
    rupiahOff: 'Rupiah taken off',
    save: 'Save',
    create: 'Create voucher',
    phoneRequired: 'The phone number is required.',
    customerNotFound: 'No customer with that number was found.',
    searchFailed: 'Could not search for the customer.',
    grantFailed: 'Could not grant the voucher.',
    alreadyHas: 'The customer already has this voucher.',
    noName: '(no name)',
    deactivateFailed: 'Could not deactivate the voucher.',
    emptyBody: 'Create the first voucher for your customers.',
    noneCreated: 'No vouchers have been created yet.',
    inactive: 'Inactive',
  },
  settlement: {
    expectedLabel: 'Expected deposit',
    expectedUnavailable: 'Cannot be read yet',
    shortWarning: '{amount} short of what is expected. A shortfall is deducted from your pay.',
    overWarning: '{amount} more than expected. Check the count again.',
    checkOutFirst:
      'Finish and check out of your shift first. The deposit appears after you check out.',
    viewHistory: 'View deposit history',
    countHint:
      'Count all the COD cash you collected this shift, then enter the amount you hand to the cashier.',
    handToCashier: 'Hand over to the cashier',
    cashierMatchHint: 'The cashier will match your deposit against the COD total.',
    done2: 'Done',
    title: 'Cash settlement (COD)',
    nothingToSettle: 'No shift to settle',
    shiftDone: 'Shift finished',
    willSettle: 'To hand over',
    recorded: 'Settlement recorded',
    awaitingCashier: "Awaiting the cashier's check",
    codTotal: 'Total COD collected',
    youHandedOver: 'You handed over',
    historyAria: 'Settlement history',
    amountLabel: 'Amount handed to the cashier',
    loadFailed: 'Could not load',
    settleFailed: 'Could not settle. Try again.',
    short: 'Short',
    over: 'Over',
    difference: 'Difference',
  },
  earnings: {
    thisMonth: 'This month',
    cancel2: 'Cancel',
    emptyBody2: 'Delivery fees appear here once you complete a delivery.',
    title: 'Earnings',
    available: 'Available balance',
    shiftTarget: 'Shift target',
    // O10: the door to the deposit flow, which lived only on the Profile screen.
    settleCash: 'Deposit cash',
    withdraw: 'Withdraw',
    overBalance: 'That is more than the available balance.',
    breakdown: 'Breakdown',
    withdrawHistory: 'Withdrawal history',
    amount: 'Withdrawal amount',
    account: 'Destination account',
    accountHint: 'BCA ···· 4821',
    emptyTitle: 'No earnings yet',
    processing: 'Processing',
    sent: 'Sent',
    failed: 'Failed',
    loadFailed: 'Could not load',
    withdrawFailed: 'Could not withdraw. Try again.',
  },
  expenses: {
    rejectedReason: 'Reason: {reason}',
    receipt: 'Receipt photo',
    receiptHint: 'Required for automatic approval. Without one the claim waits for the depot.',
    autoApprove: 'Claims up to {amount} are approved automatically; above that a manager decides.',
    autoApproveUnknown: 'Small claims are approved automatically; the rest wait for a manager.',
    receiptPicked: 'Receipt ready to upload.',
    submitClaim: 'Submit claim',
    emptyBody2: 'Expense claims you submit will appear here.',
    title: 'Expense claims',
    history: 'Claim history',
    kind: 'Expense type',
    amount: 'Amount',
    note: 'Description',
    noteHint: 'e.g. fuel for the morning shift',
    emptyTitle: 'No claims yet',
    fuel: 'Fuel',
    parking: 'Parking / tolls',
    service: 'Vehicle service',
    other: 'Other',
    pending: 'Pending',
    approved: 'Approved',
    rejected: 'Rejected',
    submitFailed: 'Could not submit the claim. Try again.',
  },
  driverPerf: {
    title: 'Weekly performance',
    depotRank: 'Depot ranking this week',
    perDay: 'Deliveries per day',
    prevWeek: 'Previous week',
    nextWeek: 'Next week',
    completed: 'Deliveries completed',
    rating: 'Rating',
    onTime: 'On time (SLA)',
    failedDeliveries: 'Failed deliveries',
  },
  cashierShift: {
    openFirst: 'Open a shift first — counter sales are refused while nobody owns the drawer.',
    closeShift2: 'Close shift',
    cancel2: 'Cancel',
    closed: 'Shift closed',
    expected: 'Expected',
    counted: 'Counted',
    difference: 'Difference',
    noneOpen: 'No shift is open',
    openingFloat: 'Opening float in the drawer',
    countedCash: 'Cash counted in the drawer',
    countedHint: 'Count it first, then enter it. The server works out the difference.',
    noteOpt: 'Note (optional)',
    noteHint: 'Change was short by Rp 2,000',
    opened: 'Shift opened.',
    openFailed: 'Could not open the shift.',
    closeFailed: 'Could not close the shift.',
  },
  operatorSummary: {
    notSettled: '{count} couriers have not settled',
    lowStockCount: 'Low stock · {count}',
    viewQueue2: 'View the queue',
    noUnassigned: 'No orders are waiting to be assigned.',
    verifyDeposit: 'Verify deposit',
    pickDepot2: "Pick a depot you run to see today's summary.",
    today: 'Today at a glance',
    needsAssigning: 'Needs assigning',
    assign: 'Assign',
    allAboveThreshold: 'Every item is above its threshold.',
    settlementsPending: 'Settlements waiting',
    ordersIn: 'Orders received',
    activeCouriers: 'Active couriers',
    codOutstanding: 'COD not yet settled',
    pickDepot: 'Select a depot',
  },
  depotHours: {
    add2: 'Add',
    cancel2: 'Cancel',
    breakStart: 'Break starts',
    breakEnd: 'Break ends',
    title: 'Opening hours &amp; closures',
    openHours: 'Opening hours',
    break: 'Break',
    closed: 'Closed',
    holidays: 'Closures',
    noExceptions: 'No schedule exceptions yet.',
    noteHint: 'Description',
    deleteAria: 'Delete',
    saveFailed: 'Could not save the opening hours.',
  },
  depots: {
    contact: 'Depot contact',
    payment: 'Depot payment details',
    codeHint: 'JKT-01',
    nameHint: 'Cikini depot',
    cityHint: 'Central Jakarta',
    provinceHint: 'DKI Jakarta',
    addressHint: 'Jl. Cikini Raya No. 1',
    whatsapp: 'Depot WhatsApp number',
    whatsappHint:
      'Where the midday and evening sales reports go. Digits only (a leading + is fine). Blank sends them to the head-office ops number.',
    bankName: 'Bank name',
    bankNameHint: 'e.g. BCA',
    bankAccount: 'Account number',
    bankHolder: 'Account holder',
  },
  pod: {
    sealQuestion: 'Gallon seal condition',
    sealBroken: 'Seal broken',
    discardTitle: 'Discard this proof of delivery?',
    cancel: 'Cancel',
    discardConfirm: 'Discard this proof of delivery? The photo, recipient name and signature you have filled in will be lost.',
    sealIntact: 'Gallon seal intact and not leaking',
    sealLabel: 'Seal',
    sealYes: 'Intact at handover',
    sealNo: 'Broken at handover',
    clearSignature: 'Clear signature',
    heading: 'Proof of delivery · {order}',
    previewAlt: 'Photo preview',
    photo: 'Delivery photo',
    optional: '(optional)',
    recipient: "Recipient's name",
    recipientHint: 'e.g. Budi Santoso',
    noteOpt: 'Note (optional)',
    noteHint: 'e.g. Handed straight to the customer',
    readFailed: 'Could not read the image',
    photoFirst: 'Take the proof-of-delivery photo first.',
    sealFirst: 'Choose the gallon seal condition first.',
    nameRequired: "Enter the recipient's name.",
    finishFailed: 'Could not complete the delivery.',
    replacePhoto: 'Replace photo',
    takePhoto: 'Take photo',
    sending: 'Sending…',
    finish: 'Complete delivery',
  },
  poDetail: {
    arrivingNow: 'Arriving now',
    arrivingFor: 'Quantity arriving for {label}',
    receivedSoFar: '{received} of {ordered} received',
    shortfallRecorded: 'Short delivery: {note}',
    shortfallPlaceholder: 'Why the rest is not coming',
    shortfallFor: 'Short-delivery reason for {label}',
    receivedAt: 'Received on {at}.',
    receiveHintPost: 'into the depot inventory for every line.',
    receiveHintPre: 'Receiving goods records',
    backToList: 'Back to the list',
    sendToSupplier: 'Send to supplier',
    receiveGoods: 'Receive goods → RECEIPT',
    gateBody2: 'Procurement is for depot managers and super admins.',
    items: 'Items',
    qtyPrice: 'Qty × price',
    subtotal: 'Subtotal',
    shipping: 'Shipping',
    total: 'Total',
    notFound: 'Purchase order not found',
    backAria: 'Back',
    managerOnly: 'Depot managers only',
    draft: 'Draft',
    sent: 'Sent',
    received: 'Received',
    actionFailed: 'That did not work. Try again.',
  },
  approvalDetail: {
    approve: 'Approve',
    reject: 'Reject',
    deciding: 'Working…',
    alreadyRejected: 'This item has already been rejected.',
    alreadyApproved: 'This item has already been approved.',
    backToList2: 'Back to the list',
    note2: 'Note',
    notFound: 'Approval not found',
    backAria: 'Back',
    system: 'System',
    physical: 'Physical',
    difference: 'Difference',
    gallonCondition: 'Gallon condition',
    deposit: 'Deposit',
    surplus: 'Surplus gallons',
    depositValue: 'Deposit value',
    expected: 'Expected',
    received: 'Received',
  },
  announcements: {
    scheduledNotSent: 'Scheduled {at} — not sent yet',
    sentTo: 'Sent {at} to {n} people',
    mergedTargets: 'Targets are merged. Someone in two targets still receives one message.',
    delete2: 'Delete',
    addTarget: 'Add target',
    title: 'Announcements',
    subtitle: 'One message, delivered once. Overlapping targets do not send twice.',
    history: 'History',
    empty: 'No announcements yet.',
    scheduled: 'Scheduled',
    compose: 'Write an announcement',
    audience: 'Audience',
    pick: 'Select…',
    subject: 'Subject',
    body: 'Message',
    level: 'Level',
    scheduleAt: 'Schedule (blank sends it now)',
    close: 'Close',
    stats: 'Statistics',
    required: 'The subject and message are required',
    targetValue: 'Every target except "whole company" needs a value',
    scheduledOk: 'Announcement scheduled',
    sent: 'Announcement sent',
    sendFailed: 'Could not send the announcement',
    schedule: 'Schedule',
    sendNow: 'Send now',
  },
  hrCustomers: {
    depotLabel: 'Depot:',
    readOnly: 'Read only. The data is read straight from the depot directory.',
    customers2: 'Customers',
    name: 'Name',
    phone: 'Phone',
    tier: 'Tier',
    orders: 'Orders',
    lastOrder: 'Last order',
    searchHint: 'Search by name or number',
    searchAria: 'Search customers',
    noDepot: 'No depot yet',
    noCustomers: 'No customers yet',
  },
  myKasbon: {
    title: 'My Cash Advance',
    subtitle: 'Raise a cash advance and follow the decision.',
    amount: 'Amount (Rp)',
    amountHint: 'e.g. 500000',
    reason: 'Reason',
    reasonHint: 'What the advance is for',
    apply: 'Request advance',
    amountRequired: 'Amount must be more than 0.',
    reasonRequired: 'A reason is required.',
    submitted: 'Request sent.',
    submitFailed: 'The request could not be sent.',
    empty: 'No cash advance requests yet.',
    cancel: 'Withdraw',
    cancelConfirm: 'Withdraw this request? It leaves the approver’s queue.',
    cancelled: 'Request withdrawn.',
    cancelFailed: 'The request could not be withdrawn.',
    rejectionReason: 'Reason for refusal: {note}',
    termsHint: 'The instalment and the month it starts are set by the approver, not the applicant.',
    approvedTerms: 'Approved — the deduction shows up in the loan list and on your payslip.',
    listTitle: 'Request history',
  },
  kasbonQueue: {
    title: 'Cash Advance Queue',
    subtitle: '{n} request(s) awaiting a decision',
    empty: 'No cash advance requests at your depots.',
    reason: 'Reason',
    approve: 'Approve',
    reject: 'Reject',
    installment: 'Monthly instalment (Rp)',
    startPeriod: 'First deduction (YYYY-MM)',
    note: 'Note',
    noteRequired: 'A reason for refusal is required.',
    termsRequired: 'An instalment and a start month are required to approve.',
    approved: 'Cash advance approved.',
    rejected: 'Cash advance rejected.',
    decideFailed: 'The decision could not be saved.',
    staleHint: 'Somebody else has already answered this one. Reload the list.',
    backToLoans: 'See the running loans',
    link: 'Cash advance requests',
    allStatuses: 'All statuses',
  },
  myLeave: {
    rejectionReason: 'Reason for rejection: {note}',
    workingDays: '{days} working days',
    cancel: 'Cancel it',
    cancelConfirm:
      "Withdraw this leave request? It leaves your approver's queue, and re-filing starts the approval chain again.",
    quotaLine: 'Quota {quota} days · used {used} · left {left}',
    apply2: 'Request leave',
    holidayHint:
      'Public holidays and weekly days off inside the range do not count against your quota.',
    title: 'My leave',
    empty: 'No leave requests yet.',
    type: 'Leave type',
    start: 'Start',
    end: 'End',
    reason: 'Reason',
    reasonHint: 'Family event',
    datesRequired: 'A start and end date are required.',
    reasonRequired: 'A reason is required.',
    submitted: 'Leave request sent',
    submitFailed: 'Could not send the request.',
    cancelled: 'Request cancelled',
    cancelFailed: 'Could not cancel it',
    quotaUnreadable: 'Your remaining leave quota could not be read — reload the page.',
    subtitle: 'Request leave and follow its approval',
    deductsQuota: ' (uses quota)',
  },
  payroll: {
    statusLabel: 'Status',
    statusAll: 'All statuses',
    needDepot: 'Choose a depot first',
    batchDepot: 'Depot',
    batchPickDepot: 'Choose a depot',
    batchGenerate: 'Generate for one depot',
    batchHint: 'Writes a DRAFT for every active employee of that depot. Approving and paying stay manual, one person at a time.',
    batchDone: '{n} drafts written',
    batchFailed: 'Batch failed',
    batchGenerated: '{n} drafts written',
    batchFailedCount: '{n} employees got no draft',
    batchNoFailures: 'Everybody has a draft.',
    periodPresentCreated: '{period} · {days} days present · created {at}',
    // PG-01 — the employee record was anonymised by retention; there is no name left.
    unnamedEmployee: 'Unnamed employee (record anonymised)',
    title: 'Payroll',
    period: 'Period',
    generate: 'Generate',
    empty: 'No payroll for this period yet.',
    employeeOpt: 'Employee (optional)',
    allEmployees: 'All employees',
    needEmployeeId: 'Enter an employeeId to generate',
    generated: 'Payroll generated',
    generateFailed: 'Could not generate it',
  },
  allowances: {
    fixedHint: 'A fixed component paid every period. It is not part of the overtime wage base.',
    addAllowance2: 'Add allowance',
    title: 'Allowances',
    empty: 'No allowances yet.',
    type: 'Type',
    amountPerMonth: 'Amount per month (Rp)',
    start: 'From',
    end: 'Until (blank means open-ended)',
    noteOpt: 'Note (optional)',
    amountPositive: 'The allowance must be greater than 0.',
    startRequired: 'A start date is required.',
    added: 'Allowance added',
    saveFailed: 'Could not save.',
    stopFailed: 'Could not stop the allowance',
    running: 'Running',
    stopped: 'Stopped',
  },
  loans: {
    stopFailed: 'Could not stop the advance',
    title: 'Loans & advances',
    empty: 'No loans yet.',
    stop: 'Stop',
    add: 'Add loan',
    principal: 'Principal (Rp)',
    installment: 'Instalment per month (Rp)',
    startPeriod: 'Starting period',
    noteOpt: 'Note (optional)',
    principalPositive: 'The principal must be greater than 0.',
    installmentPositive: 'The monthly instalment must be greater than 0.',
    added: 'Loan added',
    count: '{n} loans',
    activeOnly: 'Still running only',
    importLink: 'Import running loans →',
    terms: 'From {period} · installment',
    remaining: 'Remaining',
    settled: 'Settled',
    running: 'Running',
    stopped: 'Stopped',
  },
  promotions: {
    new: 'New promotion',
    cancel2: 'Cancel',
    emptyBody: 'Create the first promotion to show on the customer home page.',
    delete2: 'Delete',
    deniedBody: 'Promotions are for the marketing team and depot leads.',
    newPromo: 'New promotion',
    headline: 'Headline',
    headlineHint: 'Free delivery on your first order',
    subhead: 'Subheading',
    subheadHint: 'For first orders',
    imageUrl: 'Image (URL)',
    voucherCode: 'Voucher code',
    ctaLabel: 'Button label',
    ctaLabelHint: 'Order now',
    ctaHref: 'Button link',
    sortOrder: 'Order',
    active: 'Active',
    activeHint: 'Switch off to hide it from the home screen',
    startsAt: 'Starts',
    startsHint: 'Leave blank to start immediately',
    endsAt: 'Ends',
    endsHint: 'Leave blank for no end date',
    denied: 'Access denied',
    empty: 'No promotions yet',
    headlineRequired: 'The headline is required.',
    saveFailed: 'Could not save the promotion.',
    edit: 'Edit promotion',
    save: 'Save',
    create: 'Create promotion',
    inactive: 'Inactive',
    deleteFailed: 'Could not delete the promotion.',
  },
  walkIn: {
    total: 'Total',
    paymentMethod: 'Payment method',
    voucherHint:
      "The buyer's own voucher. Needs a phone number; the server works out the discount when it saves.",
    cashShort: 'The cash is less than the total.',
    payTargetUnreadable:
      "The depot's payment details could not be read — reload the page before accepting QRIS or a transfer.",
    noPayTarget: 'This depot has not set up payment details for that method.',
    phoneFirst: "Enter the buyer's phone number first — a voucher belongs to an account.",
    orderSavedNoPayment:
      'The order is saved but the payment is not recorded — record it before the buyer leaves.',
    // K3.2: said on the till, not in a toast. The cashier is holding the money right now.
    unpaidTitle: 'Payment not recorded yet',
    unpaidBody:
      'Sale {order} is saved, but its payment was not recorded. The money is in your drawer — record it now, while the buyer is still here.',
    retryPayment: 'Try recording it again',
    paymentSettled: 'Payment for {order} is recorded.',
    receiptBlocked: 'The receipt could not open — allow pop-ups, then press "Reprint receipt".',
    cash: 'Cash',
  },
  purchaseOrders: {
    value: 'Value',
    title: 'Purchase orders',
    noDepot: 'No depot yet',
    empty: 'No purchase orders yet',
    managerOnly: 'Depot managers only',
    draft: 'Draft',
    sent: 'Sent',
    received: 'Received',
    actionFailed: 'That did not work. Try again.',
  },
  earningsHistory: {
    typeIncentive: 'Bonus',
    title: 'Earnings history',
    empty: 'Nothing recorded yet',
    emptyBody: 'Every payment, deduction and withdrawal will appear here.',
    typeEarning: 'Payment',
    typeDeduction: 'Deduction',
    typeCashVariance: 'Deposit variance',
    typeWithdrawal: 'Withdrawal',
    typeAdjustment: 'Adjustment',
    prev: 'Previous',
    next: 'Next',
    pageOf: 'Page {page} of {total}',
    seeAll: 'See all',
  },
  settlementHistory: {
    detailError: 'Could not open this settlement.',
    detailVariance: 'Variance',
    detailCharged: 'Charged to you',
    detailChargedYes: 'Yes',
    detailChargedNo: 'No',
    detailOrders: 'Orders',
    detailVerified: 'Verified',
    detailNote: 'Cashier note',
    open: 'See details',
    emptyBody2: 'Your COD cash deposits will appear here.',
    title: 'Settlement history',
    youHandedOver: 'You handed over',
    totalDue: 'Total due',
    chargedToYou: 'The difference is charged to you.',
    empty: 'No settlements yet',
    awaiting: 'Awaiting verification',
    verified: 'Verified',
    disputed: 'Disputed',
    short: 'Short',
    over: 'Over',
  },
  enroll: {
    title: 'Enrol your face',
    subtitle: 'Take 1–3 clear photos of your face for attendance',
    reset: 'Reset',
    save: 'Save',
    done: 'Your face is enrolled. You can clock in now.',
    enrolled: 'Face enrolled',
    enrollFailed: 'Enrolment failed',
  },
  commission: {
    depotNoteAfter: ', which head office computes from net profit and pays to the franchise owner.',
    depotNote:
      "This is the per-delivery commission for your depot's couriers — paid out of depot cash. It differs from",
    deliveredPaid: '{delivered} deliveries · paid {gross}',
    unpaidDeliveries: '{n} deliveries are not in the courier payment ledger',
    payoutUnavailable:
      'The courier payment figures cannot be read right now, so this period’s commission is not shown. This screen does not work them out itself — a figure computed here once differed from what was actually paid.',
    emptyBody: 'No completed deliveries for this depot in {month}.',
    gateBody2: 'Courier commission payouts are for depot managers only.',
    title: 'Courier commission',
    periodTotal: 'Commission this period',
    franchiseShare: 'franchise revenue share',
    empty: 'No commission yet',
    managerOnly: 'Depot managers only',
    settlementDiff: '· settlement difference',
    // OPS-03 — see the note on the Indonesian side. The button paid nobody.
    howPaid:
      "This commission is already in each courier's payment ledger, credited as their deliveries completed. Couriers withdraw it themselves from Earnings in their app; there is nothing to press here.",
  },
  expenseClaims: {
    note: 'Note: {note}',
    viewReceipt: 'View receipt',
    approve: 'Approve',
    reject: 'Reject',
    gateBody2: 'Expense-claim approvals are for depot managers and finance.',
    title: 'Courier expense claims',
    reviewNote: 'Reviewer note (optional)',
    reviewNoteHint: 'e.g. matches the receipt',
    empty: 'No claims',
    emptyFor: 'No {status} claims yet.',
    statusLabel: 'Status',
    statusPending: 'Pending',
    statusApproved: 'Approved',
    statusRejected: 'Rejected',
    restricted: 'Managers and finance only',
    fuel: 'Fuel',
    parking: 'Parking / tolls',
    service: 'Vehicle service',
    other: 'Other',
    actionFailed: 'That did not work. Try again.',
    allDepots: 'All depots — pick one in the switcher to filter.',
  },
  meter: {
    overThreshold:
      'The variance is past the {liters} litre threshold. Check for gallons that left unrecorded, a leak, or a mistyped meter reading.',
    orderLines: '{count} order lines',
    varianceHint: '± {gallons} equivalent gallons (reference {liters} L)',
    sameDayHint:
      'This comparison suits depots that fill gallons as sales come in. If a depot fills stock first and sells it the next day, today’s meter will not match today’s sales.',
    noVolumeHint:
      'has no litre volume today, so it is left out of the comparison. Fill in the product volume in the catalog to make this variance accurate.',
    noClosedDays: 'No day has had its meter closed yet.',
    twiceHint:
      'Enter the morning reading at opening and the evening one at closing. The same form is used twice.',
    varianceHistory: 'Variance history',
    gateBody2: 'Water-meter reconciliation is for depot staff and above.',
    title: 'Water meter',
    record: 'Record a reading',
    morning: 'Morning reading (m³)',
    evening: 'Evening reading (m³)',
    rawMorning: 'Raw water, morning (m³, optional)',
    rawEvening: 'Raw water, evening (m³, optional)',
    roYield: 'RO yield',
    waterOut: 'Water out (metered)',
    waterSold: 'Water sold (recorded)',
    difference: 'Difference',
    differenceValue: 'Value of the difference',
    restricted: 'Restricted access',
    saveFailed: 'Could not save the reading.',
    pickDepot: 'Select a depot',
    saving: 'Saving…',
    save: 'Save',
  },
  wastage: {
    sourceHintBefore: 'Every wastage entry is recorded as one inventory movement of type',
    itemKinds: '{count} item kinds',
    fromAdjustment: 'from ADJUSTMENT movements',
    sourceHint:
      'with a negative delta, so stock and loss value always agree with the inventory ledger. A rupiah value only appears for items that carry a selling price.',
    empty: 'No wastage (negative ADJUSTMENT) in this period.',
    details: 'Details',
    gateBody2: 'Wastage tracking is for depot managers only.',
    title: 'Wastage',
    value: 'Wastage value',
    units: 'Units written off',
    managerOnly: 'Depot managers only',
  },
  driverHistory: {
    title: 'History',
    done: 'Completed',
    failed: 'Failed',
    empty: 'No history yet',
  },
  hrResellers: {
    depotLabel: 'Depot:',
    readOnly: 'Read only. The same registry as the depot console.',
    photo: 'Photo',
    customer: 'Customer',
    discount: 'Discount',
    monthlyTarget: 'Target per month',
    joined: 'Joined',
    status: 'Status',
    noDepot: 'No depot yet',
    empty: 'No resellers yet',
    active: 'Active',
    inactive: 'Inactive',
  },
  managerTeam: {
    subtitle2: 'Couriers assigned to your depot.',
    emptyBody2: 'Couriers assigned to this depot will appear here.',
    active2: 'Active',
    title: 'Courier team',
    empty: 'No couriers yet',
    total: 'Total couriers',
    active: 'Active',
    courier: 'Courier',
  },
  depotDetail: {
    profile: 'Profile',
    config: 'Configuration',
    stockSummary: 'Stock summary',
    stockLines: 'Stock lines',
    lowStock: 'Running low',
    address: 'Address',
    coords: 'Coordinates',
    radius: 'Service radius',
    deliveryFee: 'Delivery fee',
    minOrder: 'Minimum order',
    holidays: 'Closures',
    active: 'Active',
    inactive: 'Inactive',
  },
  hqDepotForm: {
    codeHint: 'JKT-01',
    nameHint: 'Cikini depot',
    addressHint: 'Jl. Cikini Raya No. 1',
    cityHint: 'Central Jakarta',
    provinceHint: 'DKI Jakarta',
    whatsapp: 'Depot WhatsApp number',
    whatsappHint:
      'Where the midday and evening sales reports go. Digits only (a leading + is fine). Blank sends them to the head-office ops number.',
    ownerHint: 'PT Hydromart Cikini',
  },
  monthlyReview: {
    courierDelivered: '{name} · {n} deliveries',
    forMeeting: 'for the monthly head-office meeting',
    gateBody2: 'The monthly ops review is for depot managers only.',
    title: 'Ops review',
    gallonSales: 'Gallon sales',
    profitBreakdown: 'Profit breakdown',
    governance: 'Governance',
    teamCustomers: 'Team & customers',
    managerOnly: 'Depot managers only',
    needsCostData: 'needs cost data',
    cashCost: 'cash costs',
    termCogs: 'purchases',
    termPayroll: 'payroll',
    unreadTerms: 'not read yet: {terms}',
    profitFormula: 'revenue − purchases − payroll − expenses',
    thisMonth: 'this month',
    nonCancelled: 'non-cancelled',
    orders: 'Orders',
    revenue: 'Revenue',
    avgSla: 'Average SLA',
    onTimeDeliveries: 'deliveries on time',
    noDeliveries: 'no deliveries counted yet',
    netProfit: 'Net profit',
    turnover: 'Turnover (excluding cancellations)',
    purchasesReceived: 'Purchases received',
    payrollNet: 'Payroll (net)',
    cashOut: 'Cash paid out',
    approvalsReviewed: 'Approvals reviewed',
    stocktakeVariance: 'Stocktake variance value',
    settlementVariance: 'Settlement variance',
    daysClosed: 'Days closed',
    gallonsLastMonth: 'Gallons last month',
    gallonsThisMonth: 'Gallons this month',
    difference: 'Difference',
    percentage: 'Percentage',
    avgPerDay: 'Average per day',
    turnoverShort: 'Turnover',
    topCourier: 'Top courier',
    activeCustomers: 'Active customers',
    winBack: 'Won back from churn',
  },
  driverAnnouncements: {
    emptyBody2: 'Operational notices from the depot will appear here.',
    noDepotBody: 'Announcements appear once you are assigned to a depot.',
    title: 'Announcements',
    empty: 'No announcements yet',
    unreadAria: 'Unread',
    noDepot: 'No depot assigned yet',
    urgent: 'Urgent',
    scheduled: 'Scheduled',
    info: 'Info',
  },
  deliveryDetail: {
    call: 'Call',
    receivedBy: 'Received by {name}',
    confirmPickup: 'Confirm goods collected',
    failedReason: 'Failed: {reason}',
    podRetention:
      'Proof of delivery (photo, signature, GPS) is kept for 12 months under the Indonesian data-protection law, then deleted automatically.',
    takeCashDue: 'Take {amount}',
    codBlocksFinish: 'Take the cash first — Selesai unlocks once the payment is recorded.',
    cashTaken: 'Cash received',
    startDelivery: 'Start delivery',
    arrived: 'Arrived · capture proof',
    returnEmpties: 'Return empty gallons',
    noShow: 'Nobody home',
    reschedule: 'Reschedule',
    failed2: 'Failed',
    title: 'Delivery detail',
    landmark: 'Landmark:',
    orderDetail: 'Order detail',
    statusHistory: 'Status history',
    rescheduled: 'Rescheduled',
    time: 'Time',
    assigned: 'Assigned',
    pickedUp: 'Picked up',
    delivering: 'Out for delivery',
    done: 'Completed',
    notFound: 'Not found',
    actionFailed: 'That did not work. Try again.',
  },
  myAnnouncements: {
    markRead2: 'Mark as read',
    title: 'Announcements',
    subtitle: 'News from HR for you',
    empty: 'No announcements for you yet.',
    new: 'New',
    markFailed: 'Could not mark it as read.',
  },
  myPayroll: {
    daysAndDate: '{days} days · {date}',
    title: 'My payslips',
    empty: 'No payslips yet',
    emptyBody: 'Your payslips will appear here.',
  },
  managerPricing: {
    inactiveProduct: 'Product no longer active',
    toggleLabel: 'Turn the price rule for {product} on or off',
    enableTitle: 'Switch this price rule on?',
    disableTitle: 'Switch this price rule off?',
    enableConfirm: 'The {rule} rule starts pricing this depot from the next order onward.',
    disableConfirm:
      'The {rule} rule stops applying immediately and this depot falls back to catalogue prices from the next order. Head office approved this rule.',
    subtitle: 'Turn depot pricing rules on or off.',
    noDepots2: 'No depot has been configured yet.',
    emptyBody2: 'Depot pricing rules will appear here.',
    title: 'Dynamic pricing',
    noDepot: 'No depot yet',
    empty: 'No pricing rules yet',
    everyDay: 'Every day',
    allDay: 'all day',
    updateFailed: 'Could not update the rule.',
    allProducts: 'All products',
  },
  campaignReport: {
    title: 'Campaign report',
    total: 'Total',
    sent: 'Sent',
    failed: 'Failed',
    recipients: 'Recipients',
    pending: 'Pending',
  },
  depotPicker: {
    hint: 'Depot UUID',
    aria: 'Select a depot',
    depot: 'Depot',
  },
  documents: {
    view2: 'View',
    supersededTag: 'v{version} (superseded)',
    versionTag: 'v{version}',
    title: 'Documents',
    empty: 'No documents yet.',
    docType: 'Document type',
  },
  hrImport: {
    title: 'Import employees',
    upsert: 'Update employees that already exist',
  },
  employeeNew: {
    title: 'Add employee',
    subtitle: 'The HR-#### code is generated automatically',
  },
  checkIn: {
    inOk: 'Checked in',
    outOk: 'Checked out',
    inAt: 'In {at}',
    outAt: ' · Out {at}',
    lateBy: ' · {n} min late',
    title: 'Face attendance',
    checkIn: 'Check in',
    checkOut: 'Check out',
    noGps: 'This device has no GPS',
    allowGps: 'Allow location access (GPS) to clock in',
    faceUnsure: 'The face match is not confident enough. Move your head or blink, then try again.',
    offlineQueued: 'No signal. The punch is stored on the device and sent automatically later.',
    failed: 'Could not clock in',
  },
  notFound: {
    body2: 'The page you are looking for does not exist, or has moved.',
    title: 'Page not found',
    toCatalog: 'Go to the catalogue',
  },
  referral: {
    yourCode: 'Your referral code',
    shareHint:
      'Share this code. Once your friend’s first order is complete you get {referrer} points and they get {referee} — points, not money off.',
    title: 'Invite a friend',
    accountAria: 'Account',
    copyAria: 'Copy the code',
    invited: 'Invited',
    succeeded: 'Joined',
    points: 'Points',
  },
  depotMap: {
    inactive: 'Inactive',
    active: 'Active',
    empty: 'No depot has coordinates yet.',
    caption: "Positions are relative to each depot's coordinates.",
  },
  faceCapture: {
    take: 'Take photo',
    hint: 'Move your head slightly or blink while the photo is taken.',
  },
  imports: {
    loansTitle: 'Import outstanding advances',
    // CA-1-49: `CsvImport` renders `description` through `t()` now, like `title`.
    desc: {
      customers:
        'Imported numbers are registered first. Customers still sign up themselves by OTP with the same number — their account links straight to this record. If you fill in an address, the city is required; the province is optional.',
      inventory:
        'Upload an Excel or CSV file to create many stock lines at once. PRODUCT rows must carry sku (the catalogue code) or productId; raw stock rows must leave both blank. The name and unit of a PRODUCT row come from the catalogue, whatever the label column says.',
      pricing:
        'Every row becomes a proposed price override and still waits on HQ approval — none of it takes effect immediately.',
      staff:
        'Upload an Excel or CSV file to create many staff accounts at once. They sign in by OTP with the number written here, and every row also opens an employee record in HR. A number that already has an account is not duplicated — its role is updated. The depot column is required for Courier and Depot Head; fill dailyRate for daily pay, monthlyRate for monthly.',
      adjustments:
        'Upload wage deductions for one period. Every row is added as written — two MANUAL deductions of the same amount in one month can both be legitimate, so nothing is merged automatically.',
      allowances:
        'Upload fixed allowances (transport, meals, position) for many employees at once. Every row is added — uploading the same file twice produces duplicate allowances.',
      assets:
        'Registers company assets in bulk, including ones an employee already holds. The holder must be at the same depot as the asset; otherwise the asset is still registered but not yet handed over.',
      employees:
        'Upload an Excel or CSV file to add many employees at once. Every new row also gets a login account (OTP) per the role column, and is linked straight to the depot written there.',
      leaveBalances:
        'Carries leave quota and leave already taken over from the old system. Without it, switching systems mid-year makes everybody look like they have a full quota again. A year that already has a balance is overwritten.',
      loans:
        'For carrying an outstanding advance over from the old system. Put the REMAINING unpaid amount as at startPeriod in the principal column, not the original loan — payroll counts forward from that figure, so the original would be deducted twice.',
    },
    gateTitle: 'Bulk import is not available for this role',
    gateBody: 'A bulk import changes many rows at once, so it needs the same permission as changing them one by one. Ask your supervisor if you need it.',
    resellers: 'Import resellers / agents',
    resellersBody:
      'A phone with no account yet is pre-registered first, then listed as an agen of this depot. An agen already on the registry is UPDATED from this file — discount, target and flat gallon price. The join date is left alone.',
    staff: 'Bulk staff import',
    deductions: 'Import deductions',
    allowances: 'Import allowances',
    assets: 'Import assets',
    loans: 'Import loans',
    pickDepot: 'Pick one depot in the depot switcher first',
    loadingDepots: 'Loading depots…',
    customers: 'Import customers',
  },
  importsInventory: {
    title: 'Import stock',
  },
  importsPricing: {
    title: 'Import depot prices',
  },
  errorPage: {
    body2: 'Something unexpected went wrong. Try again, or go back to the catalogue.',
    title: 'Something went wrong',
    retry: 'Try again',
  },
  favorites: {
    browseProducts: 'Browse products',
    title: 'Favourites',
    empty: 'No favourites yet. Tap the heart on a product to save it.',
    accountAria: 'Account',
  },
  hrAllowances: {
    title: 'Allowances',
    subtitle: 'Fixed recurring pay — listed separately from bonuses on the payslip',
  },
  audit: {
    title: 'Audit log',
    filterHint: 'Filter by entity (employees, payroll, …)',
    entries: '{n} entries',
    actor: 'By',
    actorSystem: 'system',
    empty: 'No entries yet.',
  },
  employeeEdit: {
    title: 'Edit employee',
  },
  balancesImport: {
    title: 'Import opening leave balances',
  },
  managerApprovals: {
    subtitle2: "Requests that need a manager's approval.",
    allDone: 'Every request has been dealt with.',
    title: 'Approvals',
    empty: 'No approvals',
  },
  liveNav: {
    gpsOff: 'GPS is off — turn on location permission for live tracking.',
    arrived2: 'Arrived · capture proof',
    locating: 'Finding your location…',
  },
  managerShell: {
    notManager:
      'This account is not a depot manager. Sign in with a manager account to open this console.',
    managerOnly: 'Depot managers only',
  },
  offlineBanner: {
    pendingCount: '{count} items not sent yet',
    discardTitle: 'Discard this?',
    discardConfirm:
      'Discard "{kind}" from the queue? This is the only copy — it is not retried and never reaches the server. If it is a COD confirmation, the cash was taken and will never be recorded.',
    discardConfirmLabel: 'Discard',
    sendNow2: 'Send now',
    clearAria: 'Clear the offline data',
  },
  operatorShell: {
    notifAria: 'Notifications',
    notifUnreadAria: 'Notifications ({n} unread)',
    title: 'Depot operator',
  },
  receipt: {
    subtitle: 'Order receipt',
    cashier: 'Cashier',
    shift: 'Shift',
    title: 'Receipt',
    discount: 'Discount',
    cash: 'Cash',
    change: 'Change',
    method: 'Method',
    subtotal: 'Subtotal',
    deliveryFee: 'Delivery fee',
    total: 'Total',
    thanks: 'Thank you for ordering from Hydromart.',
  },
  depotCustomers: {
    gallonNotLinked: 'Depot gallon data is not connected yet',
    notLinked: 'not connected',
  },
  staff: {
    nameHint: 'e.g. Budi',
    plateHint: 'e.g. B 1234 ABC',
  },
  myAttendance: {
    title: 'My attendance',
    emptyBody: 'Your attendance will appear here.',
    empty: 'No attendance yet',
  },
  payrollDetail: {
    approvedAt: 'Approved',
    paidAt: 'Paid',
    pendingWarning: '{n} attendance day(s) in this period are still undecided. The payroll can still be approved — decide those days, then record the difference as an adjustment in the next period (deducted if rejected, paid in arrears if approved).',
    approveConfirmPending: 'Approve a payment of {net}? {n} attendance day(s) are still undecided; the difference must be recorded in the next period.',
    failed: 'Failed',
    downloadFailed: 'Download failed',
    netPay: 'Net pay',
    approved: 'Payroll approved',
    approveConfirm:
      'Approve this payroll run of {net}? Once approved, the figures can no longer be edited from this screen.',
    markPaidConfirm:
      'Mark this {net} payroll as paid? This states the transfers have left the bank, and it cannot be undone here.',
    markPaid: 'Mark as paid',
    markedPaid: 'Marked as paid',
    deduction: 'Deductions',
    approve: 'Approve',
    presentDays: '{days} days present',
    downloadPdf: 'Download PDF',
  },
  loginPage: {
    avgDelivery: 'average delivery',
    partnerDepots: 'partner depots',
  },
  staffInvite: {
    nameHint: 'e.g. Budi',
    positionHint: 'e.g. Courier',
  },
  employeeSelect: {
    label: 'Employee',
    placeholder: 'Select an employee…',
    searchPlaceholder: 'Type a name or employee code…',
    noMatch: 'No matching employee.',
    loadFailed: 'Could not load the employee list.',
  },
  accountEdit: {
    emailHint: 'name@email.com',
    phone: 'Phone number',
    phoneIsLogin: 'This is the number you sign in with. Changing it signs you out everywhere.',
    changePhone: 'Change number',
    newPhone: 'New phone number',
    sendCode: 'Send code',
    codeSentTo: 'A code was sent to {{phone}}. Enter it below.',
    code: 'Verification code',
    confirmChange: 'Confirm the change',
    cancelChange: 'Cancel',
    changed: 'Your number has been changed. Sign in again with the new one.',
    sendFailed: 'Could not send a code to that number.',
    confirmFailed: 'Could not change the number.',
    newPhoneRequired: 'Enter the new phone number first.',
    codeRequired: 'Enter the code sent to your new number.',
  },
  customerDetail: {
    pickDepot: 'Pick a depot in the switcher.',
    notLinked: '— not connected',
  },
  depotReports: {
    time: 'Time',
    cancelled: 'Cancelled',
    recipient: 'Recipient',
    subtotal: 'Subtotal',
    deliveryFee: 'Delivery fee',
    discount: 'Discount',
    orderNo: 'Order no.',
    status: 'Status',
    courier: 'Courier',
    gallons: 'Gallons',
    total: 'Total',
    counterSale: 'Counter sale',
    staffOnly: 'Depot staff only',
  },
  depotSubscriptions: {
    daily: 'Every day',
    every3Days: 'Every 3 days',
    weekly: 'Every week',
    every2Weeks: 'Every 2 weeks',
    monthly: 'Every month',
    help: 'Pick a registered customer, then choose the product and gallon count (1 or more).',
    loadingCustomers: 'Loading customers…',
    pickCustomer: 'Pick a registered customer',
    noName: 'No name',
    statusFailed: "Could not change the subscription's status.",
    notScheduled: 'not scheduled yet',
    nextRun: 'next {at}',
  },
  depotTargets: {
    mustBeNumbers: 'Every target must be a number.',
  },
  hqCatalog: {
    unitHint: 'gallon / box / pcs',
  },
  reconciliation: {
    amountIdr: 'Amount (IDR)',
    fileFailed: 'Could not build the file.',
  },
  reportsExport: {
    fileFailed: 'Could not build the file. Try the CSV format.',
  },
  retention: {
    yearsHint: '7 years (Indonesian data-protection law)',
  },
  employeeDetailExtra: {
    faceEnrolled: 'Face enrolled',
    faceFailed: 'Face enrolment failed',
    unreadable: 'Could not be read',
  },
  myPayrollDetail: {
    slipTitle: 'Payslip {period}',
    presentDays: '{n} days present',
    downloadPdf2: 'Download PDF',
    downloadFailed: 'Download failed',
    netPay: 'Net pay',
  },
  settingsExtra: {
    resetConfirm:
      "Remove this override for {scope}? The value falls back to the wider rule — a different number, applied to everyone's pay, with no undo on this screen.",
    overrideRemoved: 'Override removed',
    resetFailed: 'Reset failed',
  },
  approvalDetailExtra: {
    submittedAt: 'Submitted',
    opnameVariance: 'Stocktake variance',
    depositRefund: 'Gallon deposit refund',
    codShortfall: 'COD settlement shortfall',
    returnVariance: 'Gallon return variance',
    actionFailed: 'That did not work. Try again.',
    value: 'Value',
  },
  managerHome: {
    gallonsDelivered: 'Gallons delivered',
    yourDepot: 'Your depot',
    loadFailed: 'Could not load',
  },
  assets2: {
    manageAssets: 'Manage assets',
    none: 'No company property is held.',
  },
  documents2: {
    saved: 'Document saved',
    pickFile: 'Choose a file first.',
    uploadFailed: 'Could not upload the document.',
  },
  checkoutFix: {
    depotOnBreak:
      'The depot is on a break — express delivery is unavailable. Scheduled orders still work.',
    depotClosed:
      'The depot is closed — express delivery is unavailable. Scheduled orders still work.',
    agentPrice: 'Agent price',
    computedAtOrder: 'worked out when the order is placed',
  },
  orderDetailPay: {
    noBankAccount: 'The depot has not set up a bank account. Contact the depot.',
    noQris: 'The depot has not set up QRIS. Contact the depot.',
  },
  documents3: {
    expiresOpt: 'Valid until (optional)',
  },
  faceCapture2: {
    retry: 'Try again',
    retryHint: 'If camera access was denied, re-enable it in your browser settings, then press Try again.',
    cameraDenied: 'The camera is unavailable. Allow camera access, then reload.',
  },
  nav: {
    dashboard: 'Dashboard',
    allScreens: 'All HR screens',
    closeMenu: 'Close menu',
    more: 'More',
    aria: 'HR navigation',
    employees: 'Employees',
    departments: 'Departments',
    customers: 'Customers',
    resellers: 'Resellers / agents',
    attendance: 'Attendance',
    leave: 'Leave requests',
    payroll: 'Payroll',
    adjustments: 'Bonuses & deductions',
    allowances: 'Allowances',
    loans: 'Loans',
    assets: 'Assets',
    announcements: 'Announcements',
    rules: 'Bonus rules',
    performance: 'Performance',
    shift: 'Shifts & rotations',
    calendar: 'Working calendar',
    reports: 'Reports',
    settings: 'Payroll configuration',
    audit: 'Audit log',
    me: 'My record',
  },
  weekdayLong: {
    mon: 'Monday',
    tue: 'Tuesday',
    wed: 'Wednesday',
    thu: 'Thursday',
    fri: 'Friday',
    sat: 'Saturday',
    sun: 'Sunday',
  },
  opsNav: {
    summary: 'Overview',
    queue: 'Queue',
    inventory: 'Inventory',
    forecast: 'Forecast',
    pricing: 'Pricing',
  },
  mgrNav: {
    home: 'Home',
    approvals: 'Approvals',
    notifications: 'Alerts',
    team: 'Team',
    account: 'Account',
  },
  opNav: {
    summary: 'Overview',
    queue: 'Queue',
    sales: 'Counter sales',
    couriers: 'Couriers',
    inventory: 'Inventory',
    returns: 'Returns',
    settlements: 'Settlements',
    customers: 'Customers',
    incidents: 'Incidents',
    promotions: 'Promotions',
    rewards: 'Rewards',
    broadcast: 'Broadcast',
    myAttendance: 'My attendance',
    shift: 'Shift',
    huddle: 'Huddle',
    handover: 'Handover',
    maintenance: 'Maintenance',
    depots: 'Manage depots',
    payments: 'Payments',
    notifications: 'Notifications',
    reports: 'Reports',
    audit: 'Audit',
    settings: 'Settings',
  },
  mgrAccount: {
    openOnDesktop: 'Open on desktop',
    pnl: 'P&L report',
    purchaseOrders: 'Purchase orders',
    manageTeam: 'Manage the team',
  },
  biometric: {
    openSaved: 'Open saved session',
  },
  nativeBridge: {
    openPlayStore: 'Open Play Store',
  },
};
