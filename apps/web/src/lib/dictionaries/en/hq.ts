...
export const hq = {
  ...
  vouchers: {
    ...
    requests: {
      title: 'Depot voucher requests',
      pending: '{n} awaiting approval',
      empty: 'No pending voucher requests.',
      emptyWhy: 'This queue cannot fill yet: no depot screen raises a voucher request. The role that would raise one (MANAGER) can already create vouchers directly at /dashboard/vouchers, so this path is waiting on a decision: should depot vouchers go through HQ approval, or should this screen go.',
      by: 'By #{who} · {depot}',
      approve: 'Approve',
      reject: 'Reject',
      approved: 'Voucher {code} created',
      rejected: 'Request for {code} rejected',
      off: '{off}% off',
      offFixed: 'Rp {off} off',
    },
    list: {
