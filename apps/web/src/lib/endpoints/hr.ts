...
export const hr = {
  ...
  hr: {
    ...
    /*
     * CA-1-34: every loan on the books, paged and depot-scoped.
     *
     * The import wizard could put five hundred kasbon rows into the ledger in one paste and
     * no screen listed them — the only way to see a loan was to know whose it was and open
     * that employee.
     */
    allLoans: (q: { page?: number; pageSize?: number; activeOnly?: boolean } = {}) => {
      const p = new URLSearchParams();
      if (q.page) p.set('page', String(q.page));
      if (q.pageSize) p.set('pageSize', String(q.pageSize));
      if (q.activeOnly) p.set('activeOnly', 'true');
      const qs = p.toString();
    ...
    leaveQueue: (
      q: { depotId?: string; status?: string; page?: number; pageSize?: number } = {},
    ) => {
      const p = new URLSearchParams();
      if (q.depotId) p.set('depotId', q.depotId);
      if (q.status) p.set('status', q.status);
