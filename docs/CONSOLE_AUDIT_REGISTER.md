...
`TERBUKA` 228 · `SUDAH DIPERBAIKI` 51 · `DUPLIKAT` 3 · `DITOLAK` 2 · `KEPUTUSAN` 0 — kedua
...
| `scripts/check-a11y.mjs` + `apps/web/test/keyboard-and-names.test.tsx` | CI job `gate` (PR #TBD) | Empat cacat yang tidak pernah terlihat di laptop bertetikus: `<input type="file">` yang disembunyikan `display:none` (hilang dari urutan tab), `<tr onClick>` tanpa link maupun penangan tombol, pesan galat tanpa live region, dan kontrol form tanpa nama yang bisa dibaca. Anggarannya **NOL**, bukan baseline — tak satu pun dari keempatnya disengaja. Uji-dirinya menjalankan keempat aturan atas potongan sumber, termasuk `onChange={(e) => …}` yang membuat versi regex-nya salah 24 kali. |
| `scripts/check-i18n.mjs` — tiga pola tambahan | CI job `gate` (PR #TBD) | Gerbangnya melapor BERSIH sementara 49 kalimat Indonesia keras berdiri di 31 berkas. `jsxHole` membaca teks JSX yang sebaris dengan `{expr}`, daftar setter jadi `set*Error/Msg/Message/Notice`, dan `ternary2` membaca cabang KEDUA sebuah ternary — pembaca lama berhenti di grup pertama yang cocok. |
