...
| B3  | Checklist onboarding HQ: 3 dari 6 langkah = `!!d`; tak cek jam buka, komisi, harga, staf. Depot tanpa jam kehilangan "antar sekarang" diam-diam.                                                                                                                            | **SELESAI** #564 (`go-live-report`) + #569 (checklist HQ memeriksa jam buka dan pemilik waralaba)                                                                        |
...
| B5  | Grafana tanpa dasbor yang di-provision.                                                                                                                                                                                                                                     | **SELESAI** #568 (dasbor `hydromart-overview`, diprovision dan diperiksa di CI)                                                                                          |
| B6  | Kapasitas: load test 10 VU × 60 detik di runner 4c/16g (checkout p95 451 ms @84 rps, run 35538586254); tak pernah di ukuran VPS target; stack uji tanpa `connection_limit=5`, `mem_limit`, dan dengan rate limit 1e6; skenario franchise mengukur halaman kosong (0 depot). | Skenario franchise **SELESAI** #567 (kini 1 depot, 12 karyawan; p95 122 ms). Sisanya (ukuran VPS, `connection_limit`, rate limit) **TERBUKA**; run VPS-class **PEMILIK** |
...
| B11 | Repo **publik**; proteksi `main` = 7 cek wajib, tanpa review wajib, `enforce_admins` mati; berkas bukan-kode terlacak (`tmp-hr.json`, `.audit_tmp/`, `.audit-endpoints.json`, `.cov-full/`).                                                                                | Berkas **SELESAI** #566. Publik/proteksi: **PEMILIK**                                                                                                                    |
| B12 | Tak ada pemantauan saldo Zenziva (saldo habis = tak ada yang bisa login).                                                                                                                                                                                                   | **PEMILIK** — API saldo ada di `{CONSOLE_BASEURL}/api/balance/` (userkey+passkey), tetapi bentuk jawabannya belum diverifikasi di akun sungguhan; monitor belum dibangun |
