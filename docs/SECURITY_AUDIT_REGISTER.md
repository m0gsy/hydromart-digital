...
| PLAT-5 | Medium | platform · nest | `INTERNAL_SERVICE_KEY` is one unscoped, unrotatable, SUPER_ADMIN-everywhere secret — ditutup SEBAGIAN 2026-09-25: "unrotatable" closed (`INTERNAL_SERVICE_KEY_PREVIOUS` accepted alongside the current key in both guards, so a rotation is a two-step roll across all 17 services instead of a flag day). "Unscoped, SUPER_ADMIN-everywhere" tetap terbuka — menyempitkannya ke identitas per-pemanggil adalah keputusan desain akses (skema scoping apa?), bukan perbaikan mekanis, dan butuh sign-off pemilik sebelum dibangun. | PR-13 | Terbuka |
