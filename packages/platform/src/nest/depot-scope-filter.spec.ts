...
describe('depotScopeFilter', () => {
  it('forces a locked role to its own depot, ignoring/allowing only its own request', () => {
    expect(depotScopeFilter({ role: Role.MANAGER, depotId: A })).toBe(A);
    expect(depotScopeFilter({ role: Role.KEPALA_DEPOT, depotId: A }, A)).toBe(A);
  });

  it('forbids a locked role asking for another depot', () => {
    expect(() => depotScopeFilter({ role: Role.MANAGER, depotId: A }, B)).toThrow(
      ForbiddenException,
    );
  });

  it('forbids a locked role with no assigned depot', () => {
    expect(() => depotScopeFilter({ role: Role.MANAGER, depotId: null })).toThrow(
      ForbiddenException,
    );
  });
